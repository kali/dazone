...
fn main() {
    ...
    println!("{} length: {:2} strat: {:6} buckets: {:4} workers: {:4} hosts: {:2} rss_mb: {:5} \
              vmmsize_mb: {:5} utime_s: {:5} stime_s: {:5} ctime_s: {:5.01} groups: {:9} \
              inner_time: {:5.01}",
             start.format("%+"),
             length,
             &*runner.strategy,
...
impl Runner {
    ...
    {
        if self.strategy == "timely" || self.strategy == "dd" || self.strategy == "ddinc" {
            self.run_timely::<K>()
        } else {
        ...
        where K: ShortBytesArray
    {
        ...
        let t1: sync::Arc<sync::Mutex<Option<time::Timespec>>> =
        ...
        where K: ShortBytesArray
    {
        ...
        let index = root.index();
        let peers = root.peers();
        let mut bibi = self.sharded_input::<K>(index, peers);
        self.monitor.target.fetch_add(bibi.size_hint().1.unwrap_or(0), Relaxed);
        let result_to_go = result.clone();

        let (mut input, probe) = root.scoped::<u64, _, _>(move |builder| {
            ...
            let count: Collection<_, (bool, u32)> = group.map(|(_, _): (K, SaneF32)| (true, 1))
                                                         .consolidate();
            ...
            let probe = count.inspect(move |rec| {
                                 result_to_go.store((rec.0).1 as usize, Relaxed)
                             })
                             .probe();
        ...
        if self.strategy == "dd" {
            ...
            // "ddinc"
            let t1 = ::time::get_time();
            let initial_batch = bibi.size_hint().1.unwrap() * 95 / 100 / peers;
            for chunk in bibi.by_ref().take(initial_batch) {
                for item in chunk {
                    input.send(((item.0, SaneF32(item.1)), 1));
                }
            }
            ...
            while probe.le(&RootTimestamp::new(epoch)) {
                root.step();
            }
